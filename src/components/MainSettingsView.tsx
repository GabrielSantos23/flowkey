...
export const MainSettingsView: React.FC<MainSettingsViewProps> = ({
  ...
  const [recordingBinding, setRecordingBinding] =
    useState<HotkeyBinding | null>(null);
        ...
        <div className="max-w-3xl mx-auto py-8 px-6 space-y-6">
          <div className="flex flex-col items-center text-center space-y-3">
            <div className="relative group">
              <div className="absolute -inset-1 rounded-2xl bg-linear-to-br from-emerald-500/20 to-emerald-500/0 blur-sm group-hover:blur-md transition-all duration-300" />
            ...
            <div className="space-y-1.5 max-w-md">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Spotify's most essential controls and overlays at your
                fingertips. Control playback and browse your library with
                seamless global shortcuts.
              </p>
            </div>
            ...
            >
              <CollapsibleTrigger className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground font-medium transition-colors py-1 px-2.5 rounded-md hover:bg-muted/40 cursor-pointer">
                <span>
                  {showMoreDescription ? "Show Less" : "Features & Details"}
                </span>
                {showMoreDescription ? (
              ...
              <CollapsibleContent className="mt-2.5">
                <Card
                  size="sm"
                  className="text-left bg-muted/20 border-border/50"
                >
                  <CardContent className="text-[11px] text-muted-foreground space-y-2 py-3">
                    <div className="flex items-start gap-2">
                      <Sparkles className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                      <span>
                        System-wide global hotkeys with 0ms Win32 media hooks.
                      </span>
                    </div>
                    <div className="flex items-start gap-2">
                      <CommandIcon className="w-3.5 h-3.5 text-primary shrink-0 mt-0.5" />
                      <span>
                        Floating Now Playing Overlay (Default: <Kbd>Alt</Kbd> +{" "}
                        <Kbd>W</Kbd>).
                      </span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="w-3.5 h-3.5 flex items-center justify-center text-xs text-emerald-400 shrink-0 font-bold">
                        •
                      </span>
                      <span>
                        Instant playlist selector and artist radio inside
                        overlay.
                      </span>
                    </div>
                  </CardContent>
          ...
          <div>
            ...
            <Card size="sm" className="bg-card/70 border-border/60 mt-3">
              <CardContent className="flex items-center justify-between h-full py-3 px-4">
                <div className="space-y-0.5 pr-4">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Choose between the Dynamic Island top pill or classic
                    floating center window when pressing{" "}
                    <Kbd className="text-[10px] px-1 py-0.5 font-mono">
                      {bindings.find((b) => b.id === "now_playing_overlay")
                        ?.currentShortcut || "Alt+W"}
                    </Kbd>
                    .
                  </p>
                </div>
                  ...
                  <Select
                    ...
                    onValueChange={(val) => {
                      if (val) {
                        ...
                        showToast(
                          styleVal === "island"
                            ? "Style: Dynamic Island (Top Edge)"
                            : "Style: Floating Window (Center)",
                        );
                      }
                    }}
                  >
                    <SelectTrigger className="w-51.25 h-7 text-xs bg-secondary/50 border-border/60">
          ...
          {/* Commands Section */}
          <div
            className={`space-y-3 pt-1 transition-all ${isMasterDisabled ? "opacity-60" : ""}`}
          >
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="secondary"
                    className="text-[10px] px-1.5 py-0 font-mono bg-muted/60 text-muted-foreground"
                  >
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-8 pr-7 h-8 w-65 text-xs bg-card/60 border-border/60 focus-visible:border-emerald-500/50"
                />
                {searchQuery && (
                  <button
                    type="button"
            ...
            <Card
              size="sm"
              className="overflow-visible p-0 border-border/60 bg-card/80"
            >
              <Table containerClassName="overflow-visible">
                <TableHeader className="bg-muted/40">
                  <TableRow className="border-border/40 hover:bg-transparent">
                    <TableHead className="text-[11px] text-muted-foreground font-semibold h-8 pl-4">
                      Command
                    </TableHead>
                    <TableHead className="text-[11px] text-muted-foreground font-semibold h-8 text-right w-28">
                      Alias
                    </TableHead>
                    <TableHead className="text-[11px] text-muted-foreground font-semibold h-8 text-right w-44">
                      Shortcut
                    </TableHead>
                    <TableHead className="text-[11px] text-muted-foreground font-semibold h-8 text-center w-20 pr-4">
                      Active
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {filteredBindings.length > 0 ? (
                    filteredBindings.map((binding, index) => {
                      ...
                      const isRecordingThis =
                        recordingBinding?.id === binding.id;
                      const isNearTop = index <= 1;
                      const badges = hotkeyService.formatShortcutBadges(
                        binding.currentShortcut,
                      );
                      const isActive = !isMasterDisabled && binding.enabled;

                      const normShortcut = binding.currentShortcut
                        ? toTauriShortcut(binding.currentShortcut).toLowerCase()
                        : "";
                      const conflictsWith = normShortcut
                        ? (conflictMap.get(normShortcut) || []).filter(
                            (name) => name !== binding.name,
                          )
                        : [];
                      const hasConflict =
                        binding.enabled && conflictsWith.length > 0;
                      const isCustomized =
                        binding.currentShortcut.trim() !==
                        binding.defaultShortcut.trim();
                      ...
                      return (
                        <TableRow
                          ...
                          className={cn(
                            ...
                            "hover:bg-white/4.5",
                            isRecordingThis &&
                              "bg-emerald-500/6 hover:bg-emerald-500/8 z-40",
                            !isActive && "opacity-45",
                          )}
                        >
                            ...
                            {isEditingAlias ? (
                              ...
                              <div className="flex justify-end">
                                <button
                                  ...
                                  disabled={isMasterDisabled}
                                  onClick={() =>
                                    handleStartEditingAlias(binding.id, "")
                                  }
                                  title="Add alias"
                          ...
                          <TableCell className="text-right py-2.5 w-44">
                            <div className="flex justify-end items-center min-w-37.5">
                              {binding.currentShortcut ? (
                                <Tooltip>
                                  <TooltipTrigger
                                    onClick={() =>
                                      !isMasterDisabled &&
                                      setRecordingBinding(binding)
                                    }
                                    className={cn(
                                      ...
                                      isRecordingThis
                                        ...
                                        : "border-border/50 hover:border-[#1ed760]/60 hover:bg-white/4",
                                    )}
                                  >
                          ...
                          <TableCell className="text-center pr-4 py-2.5 w-20">
                            <div className="flex justify-center items-center">
                              <Switch
                                size="sm"
                                checked={
                                  isMasterDisabled ? false : binding.enabled
                                }
                                disabled={isMasterDisabled}
                                onCheckedChange={() =>
                                  handleToggleEnable(
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground pt-1">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  Restore all shortcuts to factory defaults
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
