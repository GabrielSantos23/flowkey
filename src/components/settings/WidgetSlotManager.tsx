  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <h4 className="text-xs font-semibold text-white">Small Widgets Bar (Collapsed Pill)</h4>
        ...
        <div className="grid grid-cols-3 gap-2">
          {(["left", "middle", "right"] as const).map((slot) => {
            ...
            return (
              <div key={slot} className="p-2.5 rounded-xl bg-white/5 border border-white/10 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-semibold text-island-primary uppercase tracking-wider">
                    {slot}
                  </span>
                  <span className="text-[10px] text-neutral-500 font-mono">{activeList.length}</span>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center justify-between w-full p-1.5 rounded-lg text-left text-xs transition-all ${
                          isChecked
                            ? "bg-island-primary/20 text-white border border-island-primary/30"
                      ...
                      >
                        ...
                        {isChecked && <Check className="w-3 h-3 text-island-primary flex-shrink-0" />}
                      </button>
                    );
                  })}
                </div>
      ...
      <div className="space-y-2 pt-2 border-t border-white/10">
        <h4 className="text-xs font-semibold text-white">Expanded Hub Widgets</h4>
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between p-2 rounded-xl border text-xs transition-all ${
                  isChecked
                    ? "bg-white/10 border-island-primary/40 text-white"
                    : "bg-white/5 border-white/5 text-neutral-400 opacity-60 hover:opacity-100"
                }`}
              >
                <div className="flex items-center gap-2">
                  <Icon className="w-4 h-4 text-island-primary" />
                  <span className="font-medium text-[11px]">{bw.label}</span>
                </div>
                {isChecked && <Check className="w-3.5 h-3.5 text-island-primary" />}
              </button>
            );
          })}
        </div>
